...
int main(int argc, char** argv) {
    ...
    int y = 20;
    const int left_x = 20;
    const int right_x = 520;
    const int line_height = (int)(cache->scale * cache->row_size) + 8;
    ...
    while (running) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            stb_font_draw_text_formatted(cache, left_x + 50 + i * 100, y, text, &color_fmt, -1);
            
            char text2[64];
            snprintf(text2, sizeof(text2), "颜色%d", i + 1);
            stb_font_draw_text_formatted(cache, right_x + 50 + i * 80, y, text2, &color_fmt, -1);
        }
        y += line_height + 10;
        
        /* Footer with light color */
        stb_font_text_format_t footer_fmt = stb_font_format_color(200, 200, 220, 255);
        stb_font_draw_text_formatted(cache, left_x, height - 30, 
